<template>
    <div class="order">
        ...
        <div class="step-group">
            <div v-if="step === 1" class="group-1 yx-center">
                <div class="address-container" :class="(5 % 3) !== 0 ? 'fill-space' : ''">
                    <c-address :is-selected="index === selectedAddressIndex" v-for="(item, index) in addressList" :key="index"
        ...
        <c-foot></c-foot>
        <c-pay-modal v-if="showPay" :show-pay="showPay" :is-clear-arr="isClearArr"  @hide="hidePayModal" :should-pay-money="totalAmount" @password="payForOrder" @clear="isClearArr = false" @forgot="onForgotKey"/>
...
<script>
    ...
    import CTimeline from './../components/pages/order/c-timeline'
    import CAddress from '../components/public/c-address'
    import CPayModal from '../components/public/c-pay-modal'
    import CNewAddress from './../components/pages/order/c-new-address'
    import COrderSuccess from './../components/pages/order/c-order-success'
    import CGoodList from '../components/pages/cart/c-good-list'
    import CModal from '../components/public/c-modal'

    import {pCreateOrderItem, pCreateOrder, pGetGoodsInfoList, pPayForOrder} from './../js/http/param'
    ...
    export default {
        ...
        methods: {
            ...
            fetchGoodsInfo() {
                ...
                this.$api.getGoodsInfoList(pGetGoodsInfoList).then(res => {
            ...
            createOrder() {
                ...
                this.$api.createOrder(pCreateOrder).then(res => {
                    console.log(res)
            ...
            payForOrder(paykey) {
                ...
                const loading = this.$tips.tLoading('支付中，请稍后')
                this.$api.payForOrder(pPayForOrder).then(res => {
            ...
            // 检查是否已经设置了支付密码
            _checkHasPaykey() {
                this.$api.checkHasPaykey().then(res => {
