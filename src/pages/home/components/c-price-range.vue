<template>
    <div class="c-price-range">
        <span class="c-price-title">价格范围</span>
        <el-checkbox-group class="price-checkbox"
                           v-model="checked"
                           @change="onChange"
                           :min="1">
            <el-checkbox v-for="(item, key) in PRICE_RANGE" :label="item.value" :key="key">{{item.label}}</el-checkbox>
        </el-checkbox-group>
    </div>
</template>
